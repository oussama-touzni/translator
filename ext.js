function onLoaded() {
    ...
    if (readFromConfigFile("apiKey") == undefined) {
       document.getElementById("modal_initial_configuration").style.display="block";
    }
    ...
    updateLayersList();
    csInterface.addEventListener("documentAfterActivate", updateLayersList);

}

function initLanguageSelect() {
    if (readFromConfigFile("pluginLanguage")) {
        selectOption("docTranslateLanguagesFrom", readFromConfigFile("pluginLanguage"));
        selectOption("selectLanguagesQuickFrom", readFromConfigFile("pluginLanguage"));
        selectOption("layersLanguageFrom", readFromConfigFile("pluginLanguage"));
    }
    
    if (readFromConfigFile("pluginLanguageTo")) {
        selectOption("docTranslateLanguagesTo", readFromConfigFile("pluginLanguageTo"));
        selectOption("selectLanguagesQuickTo", readFromConfigFile("pluginLanguageTo"));
        selectOption("layersLanguageTo", readFromConfigFile("pluginLanguageTo"));
    }
    
}

function initSupportedLanguages() {
    if (readFromConfigFile("langCfg") == undefined && readFromConfigFile("apiKey") != undefined) {
        getSupportedLanguages();
    }
    else if (readFromConfigFile("langCfg") != undefined) {
        var languages = JSON.parse(readFromConfigFile("supportedLanguages"));
        setLanguageSelectOptions("docTranslateLanguagesFrom", languages.data.languages);
        setLanguageSelectOptions("docTranslateLanguagesTo", languages.data.languages);
        setLanguageSelectOptions("selectLanguagesQuickTo", languages.data.languages);
        setLanguageSelectOptions("selectLanguagesQuickFrom", languages.data.languages);
        setLanguageSelectOptions("pluginLanguage", languages.data.languages);
        setLanguageSelectOptions("pluginLanguageTo", languages.data.languages);
        setLanguageSelectOptions("layersLanguageTo", languages.data.languages);
        setLanguageSelectOptions("layersLanguageFrom", languages.data.languages);
        
    }
}

function initConfigTab(){
    if (readFromConfigFile("apiKey")) {
     $('#apiKey').val(readFromConfigFile("apiKey"));
     }
    if (readFromConfigFile("exportFolder")) {
     $('#exportDirectory').val(readFromConfigFile("exportFolder"));
    }
    if (readFromConfigFile("translationMode")) {
        selectOption("translationMode", readFromConfigFile("translationMode")) ;
    }
    if (readFromConfigFile("pluginLanguage")) {
        selectOption("pluginLanguage", readFromConfigFile("pluginLanguage")) ;
    }
    if (readFromConfigFile("pluginLanguageTo")) {
        selectOption("pluginLanguageTo", readFromConfigFile("pluginLanguageTo")) ;
    }
}

function selectOption(id, selectedValue) {
    for (var i = 0; i < document.getElementById(id).options.length; i++) {
        if (document.getElementById(id).options[i].value == selectedValue) {
